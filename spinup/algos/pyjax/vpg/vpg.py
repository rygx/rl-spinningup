...
def vpg(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0, 
    ...
    # VPG objectives
    pi_loss = -jnp.mean(logp * adv_ph)
    v_loss = jnp.mean((ret_ph - v)**2)

    # Info (useful to watch during learning)
    approx_kl = jnp.mean(logp_old_ph - logp)      # a sample estimate for KL-divergence, easy to compute
    approx_ent = jnp.mean(-logp)                  # a sample estimate for entropy, also easy to compute
